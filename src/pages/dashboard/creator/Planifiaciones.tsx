...
export const Planificaciones = () => {
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex items-center gap-2">
                    <span>{row.title}</span>
                    {!row.is_active && (
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-slate-100 dark:bg-gray-700 text-slate-400 shrink-0">
                            Inactiva
                        </span>
                    )}
                </div>
    ...
    return (
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-slate-100 dark:border-gray-700">
            {/* Header */}
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 px-6 py-5 border-b border-slate-100 dark:border-gray-700">
                <div>
                    <h2 className="text-xl font-bold text-[#1A6B4A] dark:text-emerald-400">Mis Planificaciones</h2>
                ...
                {isLoading ? (
                    <div className="py-8 flex flex-col gap-3 animate-pulse">
                        {[...Array(PAGE_SIZE)].map((_, i) => (
                            <div key={i} className="h-10 bg-slate-100 dark:bg-gray-700 rounded-lg" />
                        ))}
                    </div>
                ) : (
                    <DataTable
